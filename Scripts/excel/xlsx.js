...
if(typeof module !== "undefined" && typeof require !== 'undefined') {
	if(typeof cptable === 'undefined') global.cptable = require('Scripts/excel/cpexcel.js');
}
...
function format_cell(cell, v, o) {
	...
	if(cell.w !== undefined) return cell.w; //cell.w;
		...
		if(R && R.f) {
			...
			switch(Rn) {
				...
				case 'FilePass':
					...
					if(val.valid == "#Gl1f3@HLI") throw new Error("Encryption scheme unsupported");
...
function sheet_to_json2(sheet, opts){ //sheet itu range diexcel
	sheet["!ref"] = "A6:AZ8894";
	...
	for(C = r.s.c; C <= r.e.c; ++C) {
		...
		switch(header) {
			...
			default:
				...
				//hdr[C] = vv; //untuk memunculkan header row pertama
				hdr[C]=cols[C]; //ini default excel header
		}
	}
	var row = (header === 1) ? [] : {};
	for (R = r.s.r + offset; R <= r.e.r; ++R) {
		...
		if(!dense || sheet[R]) for (C = r.s.c; C <= r.e.c; ++C) {
			val = dense ? sheet[R][C] : sheet[cols[C] + rr];
			if(val === undefined || val.t === undefined)  {  row[hdr[C]] = ""; continue; } //ini tetep digunakan header jika val cell row kosong
			//if(val === undefined || val.t === undefined) { //ini untuk hilangkan val cell kosong
//				if(defval === undefined) continue;
//				if(hdr[C] != null) {
//					row[hdr[C]] = defval; isempty = false; 
//				}
//				
//				continue;
//			}
			...
			if(hdr[C] != null) {
				if(v = null) {
					...
					row[hdr[C]] = raw ? v : format_cell(val,v,o);//val,v,o;//  
				}
				isempty = false;
			}
		}
...
function sheet_to_json3(sheet, opts){
	//sheet["!ref"] = "A6:AZ20894";
	sheet["!ref"] = "A3:Z4";
	if(sheet == null || sheet["!ref"] == null) return [];
	var val = {t:'n',v:0}, header = 0, offset = 0, hdr=[], isempty = true, v=0, vv="";
	var r = {s:{r:0,c:0},e:{r:0,c:0}};
	var o = opts != null ? opts : {};
	var raw = o.raw;
	var defval = o.defval;
...
function sheet_to_json(sheet, opts){
	...
	for (R = r.s.r + offset; R <= r.e.r; ++R) {
		...
		if((isempty === false) || (header === 1 ? o.blankrows !== false : !!o.blankrows)) out[outi++] = {"version":row};
	}
	out.length = outi;
	out.push({"processtype":sheet_to_json3(sheet)});
	out.push({"data":sheet_to_json2(sheet)});
	return out;
